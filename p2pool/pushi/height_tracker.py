...
class HeaderWrapper(object):
    ...
    def from_header(cls, header):
        return cls(pushi_data.hash256(pushi_data.block_header_type.pack(header)), header['previous_block'])
...
@defer.inlineCallbacks
def get_height_rel_highest_func(pushid, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(pushid.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield pushid.rpc_getblock('%x' % (block_hash,))
